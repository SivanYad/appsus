...
import { storageServices } from "./async-storage-service.js";
...
function query() {
  return storageServices.query(BOOKS_KEY)
...
function remove(bookId) {
    ...
    return storageServices.remove(BOOKS_KEY, bookId)
}

function get(bookId) {
  return storageServices.get(BOOKS_KEY, bookId)
}

function save(book) {
  if (book.id) return storageServices.put(BOOKS_KEY, book)
  else return storageServices.post(BOOKS_KEY, book)
}

function postReview(bookId, review) {
  review.id = storageServices._makeId()
